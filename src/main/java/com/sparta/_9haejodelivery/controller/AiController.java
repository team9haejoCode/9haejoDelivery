...
public class AiController {
    ...
    @PostMapping("/")
    public ResponseEntity<ApiResponse<AiResponseDto>> aiLog(@AuthenticationPrincipal UserDetails userDetails,
                                             @Valid @RequestBody AiResponseDto requestDto) {
        User user=/*userDetails.getUser()*/null;
        return ResponseEntity.ok(ApiResponse.success(HttpStatus.CREATED, "Ai 응답이 완료되었습니다.",aiLogService.ask(user, requestDto.text)));
    }
}
